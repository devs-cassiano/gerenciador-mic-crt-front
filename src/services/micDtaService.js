...
export const micDtaService = {
  ...
  // Obter próximo número sequencial para uma transportadora específica
  getNextNumber: async (transportadoraId, paisDestinoCodigo, paisOrigemCodigo = null, licencaComplementar = null) => {
    const params = new URLSearchParams({ paisDestinoCodigo });
    
    if (paisOrigemCodigo) params.append('paisOrigemCodigo', paisOrigemCodigo);
    if (licencaComplementar) params.append('licencaComplementar', licencaComplementar);
    
    const response = await api.get(`/mic-dta/next-number/${transportadoraId}?${params}`);
    return response.data;
  },
